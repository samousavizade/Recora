...
The :class:`~recora.data.DataInfo` object stores almost all the useful information from the original data.
...
When using a ``feat`` model, the :class:`~recora.data.DataInfo` object stores the unique features of
...
The :class:`~recora.data.DataInfo` object also stores users' consumed items, which can be useful in sequence models
...
.. SeeAlso::
    ...
    `changing_feature_example.py <https://github.com/samousavizade/MyRec/blob/master/examples/changing_feature_example.py>`_
