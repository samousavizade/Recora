...
```shell
$ docker pull recora/recora:latest
```
2. Start a docker container by running following command:
```shell
$ docker run --rm -p 8889:8888 recora/recora:latest
```
...
```shell
$ docker run --rm -p 8889:8888 -v $(pwd):/root/data:ro recora/recora:latest
```
...
4. Enter `Recora` as the password.
